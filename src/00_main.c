...
/*   Updated: 2025/08/16 12:21:18 by juagomez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_data	*initialize_data(int argc, char **argv)
{
	...
	data->monit_all_alive 	= 0;			// INIT THREADS	
	data->monit_all_full	= 0;
...
void	initialize_philos(t_data *data)
{
	...
	{
		philos_ptr->id 			= index + 1;
		philos_ptr->num_meals	= 0;
		philos_ptr->state		= INITIAL;
		pthread_mutex_init(&philos_ptr->mutex_num_meals, NULL);
		pthread_mutex_init(&philos_ptr->mutex_last_eat, NULL);
		pthread_mutex_init(&philos_ptr->mutex_state, NULL);	
		//philos_ptr->data		= data;
		set_last_meal(philos_ptr);
		index++;
		philos_ptr++;
	}
}
